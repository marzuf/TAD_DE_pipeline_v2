...
# - script2: wilcox_ttest_meanTAD_qq.Rdata
...
script2_name <- "2_runWilcoxonTAD"
script3_name <- "3_runMeanTADLogFC"
script11_name <- "11_runEmpPvalCombined"
script_name <- "13EZH2_plotTopWilcoxTopCombined"
stopifnot(file.exists(paste0(pipScriptDir, "/", script_name, ".R")))
cat(paste0("> START ", script_name,  "\n"))

script_name <- "13_plotTopWilcoxTopCombined" # UPDATE 19.06.2018: reset for compatibility
...
all_tops <- c("all_wilcox_pval", "all_combined_pval")
...
###################################################################### INTERSECT WILCOX COMBINED

all_wilcox_pval_nTop <- all_wilcox_pval[1:nTopVennPlot]
all_combined_pval_nTop <- all_combined_pval[1:nTopVennPlot]

intersectReg <- intersect(names(all_wilcox_pval_nTop), names(all_combined_pval_nTop))

vennF <- paste0(curr_outFold, "/", "venn_combined_wilcox_intersect_top", nTopVennPlot, ".", plotType)
do.call(plotType, list(file=vennF, height=myHeight, width=myWidth))
draw_venn_font(vd_tit = paste0("Intersect Wilcoxon/Combined top ", nTopVennPlot, " p-values"),
               my_font ="time",  my_cex = 1.5,
               top_wilcox = names(all_wilcox_pval_nTop),
