...
# UPDATE 20.06.2018: be sure used only for microarray data; only RNAseq data: human -> ICGC has special script and inhouse RNA seq also emulate_1 script
if(!microarray) {
  stop("if not microarray should use another script (corrected version for ICGCs)!\n")
} else {
  stopifnot(! normRNAseq)  # if microarray is TRUE, RNAseq cannot be TRUE
}
