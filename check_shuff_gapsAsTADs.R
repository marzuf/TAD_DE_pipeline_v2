...
outFold <- "check_shuff_gapsAsTADs"
...
#fill_domain_gaps <- function(tadDT, chrSize) {
#   stopifnot(c("chromo", "start", "end") %in% colnames(tadDT))
#   stopifnot(max(tadDT$end) <= chrSize)
#   stopifnot(length(unique(tadDT$chromo)) == 1)
#   stopifnot(is.numeric(tadDT$start[1]))
#   stopifnot(is.numeric(tadDT$end[1]))
#   chromo <- as.character(unique(tadDT$chromo))
#   tadDT$type <- "domain"
#   gapsDT <- data.frame(chromo = chromo, 
#                        start = c(1, tadDT$end+1),
#                        end = c(tadDT$start-1, chrSize),
#                        stringsAsFactors = FALSE)
#   gapsDT <- gapsDT[gapsDT$end > gapsDT$start,]
#   # do not check start==1 and end==chrSize here, because if the true TAD starts with 1 or end with chrSize, 
#   # they are discarded at the line just here above
#   gapsDT$type <- "gap"
#   filledDT <- rbind(tadDT, gapsDT)
#   filledDT <- filledDT[order(filledDT$start),]
#   filledDT <- unique(filledDT)
#   rownames(filledDT) <- NULL
#   stopifnot(filledDT$start[1] == 1)
#   stopifnot(filledDT$end[nrow(filledDT)] == chrSize)
#   stopifnot(filledDT$end[1:(nrow(filledDT)-1)] + 1 == filledDT$start[2:nrow(filledDT)])
#   # ensure not overlapping
#   if(nrow(filledDT) > 1) {
#     for(i in 2:nrow(filledDT))
#       stopifnot(filledDT$start[i] > filledDT$end[i-1])
#   }
#   # ensure all starts smaller than ends
#   stopifnot(filledDT$start < filledDT$end)
#   return(filledDT)
# }
...
TADdt$region <- paste0(TADdt$chromo, "_TAD", 1:nrow(TADdt))
...
gene2tadDT$region <- NULL

######### assign the genes
# stopifnot(c("entrezID", "start","end", "chromo", "strand") %in% colnames(geneDT) )
# stopifnot(c("chromo", "start","end", "region") %in% colnames(regionDT) )
g2t_dt <- assignGene2TADs(regionDT=TADdt, geneDT = entrezDT, assignMethod = assignMeth)

gene2tadDT <- left_join(gene2tadDT, g2t_dt, by ="entrezID")
stopifnot(!any(is.na(gene2tadDT)))
...
stopifnot(all(grepl("TAD", gene2tadDT$region)))
...
len_obs <- length(sizeFilter_obs)
cat("... sizeFilter obs.:", len_obs, "\n")
...
outFile <- paste0(outFold, "/", "shuff_genesByTAD_", assignMeth, ".Rdata")
...
shuff_genesByTAD <- foreach(i_perm=1:nPerm) %dopar% {
  ...
  shuff_g2t_dt <- assignGene2TADs(regionDT=allChr_randTADdt, geneDT = entrezDT, assignMethod = assignMeth)
  stopifnot(nrow(shuff_g2t_dt) == nrow(tmp_gene2tadDT))
...
# load(paste0(outFold, "/", "shuff_genesByTAD_", assignMeth, ".Rdata"))

n_TADs <- sapply(shuff_genesByTAD,  length)

outFile <- paste0(outFold, "/", "nbrTADs_containing_genes_", assignMeth, ".png")
...
outFile <- paste0(outFold, "/", "nbrTADs_containing_genes_sizeFilter_", assignMeth, ".png")
png(outFile, width=600, height=600)
...
shuffList <- eval(parse(text = load("check_shuff_gapsAsTADs/shuff_genesByTAD_maxOverlap.Rdata")))

sumAssigned <- sapply(shuffList, sum)

stopifnot(length(unique(sumAssigned)) == 1)

stopifnot(unique(sumAssigned) == sum(obs_genes_TAD))

cat(".. range shuffle:\n")
range(sapply(shuffList, function(x) length(unique(names(x)))))
# 3064 3267

cat("... length observed:\n")
length(obs_genes_TAD)
# 4030
