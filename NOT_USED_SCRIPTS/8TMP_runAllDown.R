...
if(step8_for_randomTADsShuffle){
  ...
  randomShuffleDT <- eval(parse(text = load(paste0(pipOutFold, "/", script5d_name, "/permutationsDT_shuffle.Rdata"))))
  ### TAKE ONLY THE GENES FOR WHICH A LOG FC VALUE IS AVAILABLE (I.E. THE ONES USED FOR DE ANALYSIS)
  ...
  for(curr_ratio_type in allDown) {
    ...
    ratio_shuffleList <- get_statFromShuffle_para_list(DEdt=DE_topTable, shuffData=randomShuffleDT, stat_fct=allDown_fct[curr_ratio_type], ncpu=nCpu, TADonly=F)
...
if(step8_for_randomTADsGaussian){
  for(sizeRatio in percentTADsizeGaussian) {
    ...
    randomGaussianDT <- eval(parse(text = load(paste0(pipOutFold, "/", script5c_name, "/permutationsDT_", gsub("\\.","", sizeRatio), ".Rdata"))))
    ### TAKE ONLY THE GENES FOR WHICH A LOG FC VALUE IS AVAILABLE (I.E. THE ONES USED FOR DE ANALYSIS)
    ...
    for(curr_ratio_type in allDown) {
      ...
      ratio_gaussianList <- get_statFromShuffle_para_list(DEdt=DE_topTable, shuffData=randomGaussianDT, stat_fct=allDown_fct[curr_ratio_type], ncpu=nCpu, TADonly=F)
...
if(step8_for_randomTADsFix){
  for(sizeRatio in percentTADsizeFix){
    ...
    randomFixSizeDT <- eval(parse(text = load(paste0(pipOutFold, "/", script5b_name, "/permutationsDT_", gsub("\\.","", sizeRatio), ".Rdata"))))
    ### TAKE ONLY THE GENES FOR WHICH A LOG FC VALUE IS AVAILABLE (I.E. THE ONES USED FOR DE ANALYSIS)
    ...
    for(curr_ratio_type in allDown) {
      ...
      ratio_fixSizeList <- get_statFromShuffle_para_list(DEdt=DE_topTable, shuffData=randomFixSizeDT, stat_fct=allDown_fct[curr_ratio_type], ncpu=nCpu, TADonly=F)
