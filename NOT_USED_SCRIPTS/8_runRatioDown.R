...
cat("... start calculate observed ratioDown/TAD\n")
all_obs_ratioDown <- get_downByRegion_v2(gene2tadDT, DE_topTable) 
txt <- paste0(toupper(script_name), "> Number of observed ratio down computed: ", length(all_obs_ratioDown), "\n")
printAndLog(txt, pipLogFile)
save(all_obs_ratioDown, file = paste0(curr_outFold, "/all_obs_ratioDown.Rdata"))
cat(paste0("... written: ",  paste0(curr_outFold, "/all_obs_ratioDown.Rdata"), "\n"))


cat("... start calculate permutations ratioDown/TAD\n")
ratioDown_permDT <- get_statFromShuffle_para(DEdt=DE_topTable, shuffData=permutationsDT, stat="ratioDown", ncpu=nCpu, TADonly=F)
txt <- paste0(toupper(script_name), "> Number of permutations for which ratio down computed: ", ncol(ratioDown_permDT), "\n")
printAndLog(txt, pipLogFile)
txt <- paste0(toupper(script_name), "> Number of regions for which ratio down computed: ", nrow(ratioDown_permDT), "\n")
printAndLog(txt, pipLogFile)
save(ratioDown_permDT, file = paste0(curr_outFold, "/ratioDown_permDT.Rdata"))
cat(paste0("... written: ",  paste0(curr_outFold, "/ratioDown_permDT.Rdata"), "\n"))
