...
txt <- paste0(toupper(script_name), "> Plot lolli plots: ", paste0(comp_folders, collapse = ", "), "\n")
printAndLog(txt, pipLogFile)

txt <- paste0(toupper(script_name), "> Plot lolli plots for following ratios: ", paste0(toPlotLolli, collapse = ", "), "\n")
printAndLog(txt, pipLogFile)

txt <- paste0(toupper(script_name), "> Output the top # plots: ", nTopLolli, "\n")
printAndLog(txt, pipLogFile)
...
all_scores_DT <- foreach(curr_ds = comp_folders, .combine = 'rbind') %dopar% {
  ...
  names(ds_vect) <- c("dataset",  toPlotRanked)
  ds_vect  
}
...
stopifnot(paste0(ref_to_rank, "_rank") %in% colnames(all_DT))
all_DT <- all_DT[order(all_DT[,paste0(ref_to_rank, "_rank")]),]
...
ggsave(plotTable, file = plotFile)
...
for(curr_ratio_type in toPlotRanked) {
  rank_DT <- all_scores_DT[order(all_scores_DT[,paste0(curr_ratio_type, "_rank")]),]
  ...
  for(curr_ds in as.character(rownames(rank_DT))) {
    ...
    mytit <- paste0(curr_ds, " - ", curr_ratio_type, " rank ", all_scores_DT[curr_ds,paste0(curr_ratio_type, "_rank") ])
