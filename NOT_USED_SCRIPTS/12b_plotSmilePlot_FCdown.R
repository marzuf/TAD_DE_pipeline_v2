...
script8_name <- "8b_runFCdown"
script_name <- "12b_plotSmilePlot_FCdown"
...
obs_FCdown <- eval(parse(text = load(paste0(pipOutFold, "/", script8_name, "/all_obs_FCdown.Rdata"))))
permut_FCdown_DT <- eval(parse(text = load(paste0(pipOutFold, "/", script8_name, "/FCdown_permDT.Rdata"))))

### !!! NEED TO CHECK WHY THERE ARE SOME NA
permut_FCdown_DT <- na.omit(permut_FCdown_DT)

nRandom <- nRandomPermut

intersectRegions <- intersect(names(obs_FCdown), rownames(permut_FCdown_DT))
...
if(useTADonly) {
    if( length(grep("_TAD", rownames(permut_FCdown_DT))) > 0 ) {
        txt <- paste0(toupper(script_name), "> !!! WARNING: permutation FCdown data contain non-TAD regions as well !!!\n")
        printAndLog(txt, pipLogFile)    
    }
...
outFile <- paste0(curr_outFold, "/smilePlotObservExpect_by", histBreakStep, "_filter.", plotType)
outFile2 <- paste0(curr_outFold, "/other_smilePlotObservExpect_by", histBreakStep, "_filter.", plotType)

initNrow <- nrow(permut_FCdown_DT)
permut_FCdown_DT <- permut_FCdown_DT[intersectRegions,]
txt <- paste0(toupper(script_name), "> Take only the regions form permut for which genes were retained: ", nrow(permut_FCdown_DT), "/", initNrow, "\n")
printAndLog(txt, pipLogFile)

initLen <- length(obs_FCdown)
obs_FCdown <- obs_FCdown[intersectRegions]
txt <- paste0(toupper(script_name), "> Take only the regions form observ. for which genes were retained: ", length(obs_FCdown), "/", initLen, "\n")
printAndLog(txt, pipLogFile)

stopifnot(nrow(permut_FCdown_DT) == length(obs_FCdown))
...
nTAD <- sum(regexpr("_TAD",  rownames(permut_FCdown_DT)) > 0)
cat(paste0("... TADs: ", nTAD, "/", nrow(permut_FCdown_DT), "\n"))
permut_FCdown_vect <- as.vector(permut_FCdown_DT[grep("_TAD", rownames(permut_FCdown_DT)),])
stopifnot(length(permut_FCdown_vect) == ncol(permut_FCdown_DT) * nTAD)

cat(paste0("length(permut_FCdown_vect) = ", length(permut_FCdown_vect), "\n"))

cat("> Prepare observed data \n")
nTAD <- sum(regexpr("_TAD",  names(obs_FCdown)) > 0)
cat(paste0("... TADs: ", nTAD, "/", length(obs_FCdown), "\n"))
obs_FCdown <- obs_FCdown[grep("_TAD", names(obs_FCdown))]

cat("> Prepare data for plotting \n")

obs_rel_hist <- hist(obs_FCdown, breaks=seq(0,1,histBreakStep), plot=F)
#plot(obs_rel_hist)

cat(paste0("length(permut_FCdown_vect) = ", length(permut_FCdown_vect), "\n"))
cat(paste0("length(permut_FCdown_vect) = ", length(na.omit(permut_FCdown_vect)), "\n"))
shuff_rel_hist <-  hist(permut_FCdown_vect,breaks=seq(0,1,histBreakStep), plot=F)
# plot(shuff_rel_hist)

cat(paste0("ncol(permut_FCdown_DT) = ", ncol(permut_FCdown_DT), "\n"))
...
stopifnot(sum(shuff_rel_hist$counts) == length(permut_FCdown_vect))
stopifnot(sum(obs_rel_hist$counts) == length(na.omit(obs_FCdown)))
...
rel_HistValues <- apply(permut_FCdown_DT[grep("_TAD", rownames(permut_FCdown_DT)),],
                        2, function(x) hist(x,breaks=seq(0,1,histBreakStep), plot=F)$counts)
...
plot(rel_logRatio ~ rel_ycoord, type="l",axes=F, cex.main=0.9,
     main = "Smile plot",
     xlab = "% of down FC per TAD", ylab = "log2(Observed/Randomized)")
...
barplot(merged_freq, beside=T, col=c(obsCol, permutCol), 
        ylab="Frequency", xlab="% of down FC per TAD", border=NA)
...
rel_ycoord <- rel_ycoord-(histBreakStep*100)/2
my_ylab <- paste0(rel_ycoord - (histBreakStep*100)/2, "-", rel_ycoord + (histBreakStep*100)/2, "%")
xlabpos <- seq(2, 1/histBreakStep * 3, by=3)[1:halfStep]
# from 0 to (nGroup+1)*nbreaks
box(bty="l")
axis(1, at = c(xlabpos), labels=c(my_ylab))
