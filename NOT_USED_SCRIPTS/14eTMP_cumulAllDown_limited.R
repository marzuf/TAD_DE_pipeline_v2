...
script_name <- "14eTMP_cumulAllDown_limited"
...
for(curr_ratio_type in allDown_limited) {
  ...
  observ_vect <- sort(filter_obs_curr_down_half, decreasing = T)
  cumsum_obs <- cumsum(abs(observ_vect - departureFromValue))
  outFile <- paste0(curr_outFold, "/14eTMP_", curr_ratio_type, "_observ_vect.Rdata")
  save(cumsum_obs, file=outFile)
