...
#     if(withExprClass) {
#       rnaseqDT <- eval(parse(text = load(paste0(pipOutFold, "/", script1_name, "/DE_rnaseqDT.Rdata"))))
#       stopifnot(all(rownames(rnaseqDT) == geneList))
#     }
...
if(useTADonly) {
    ...
    printAndLog(txt, pipLogFile)
	#---->>>> UPDATE FOR EZH2 AS A GENE CAN MAP TO BOTH TAD AND BOUNDARY
	gene2tadDT <- gene2tadDT[grep("_TAD",gene2tadDT$region),]
}

### need to update here for EZH2 -> for the shuffling I need to assign uniquely 1 gene -> 1 TAD
# 
cat("... for EZH2 data: for genes mapping to multiple TADs, keep only the TADs with max overlap\n") 
TADposDT <- read.delim(TADpos_file, header=F, col.names = c("chromo", "region", "TAD_start", "TAD_end"), stringsAsFactors = F)
gene_tad_DT <- left_join(gene2tadDT, TADposDT, by=c("chromo", "region"))
gene_tad_DT$overlap <- pmin(gene_tad_DT$end, gene_tad_DT$TAD_end) - pmax(gene_tad_DT$start, gene_tad_DT$TAD_start) 
stopifnot(all(gene_tad_DT$overlap > 0))
gene_tad_DT_2 <- do.call(rbind, by(gene_tad_DT, gene_tad_DT$entrezID, function(x) x[which.max(x$overlap),]))
stopifnot(all(gene_tad_DT$entrezID %in% gene_tad_DT_2$entrezID))
rownames(gene_tad_DT_2) <- NULL

txt <- paste0(toupper(script_name), "> Number of rows before removing multiple TAD mapping: ", nrow(gene2tadDT), "\n")
printAndLog(txt, pipLogFile)

gene2tadDT <- gene_tad_DT_2[,colnames(gene_tad_DT_2) %in% colnames(gene2tadDT)]


txt <- paste0(toupper(script_name), "> Number of rows after removing multiple TAD mapping: ", nrow(gene2tadDT), "\n")
printAndLog(txt, pipLogFile)
