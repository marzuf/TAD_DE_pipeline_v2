...
one_of_many_rnaseqDT <- foreach(i = 1:dim(manyGenes_rnaseqDT)[1], .combine='rbind') %dopar% {
  ...
  if(any(grepl("_TAD", tmp_g2t_DT$region))) {
    tmp_g2t_DT <- tmp_g2t_DT[grep("_TAD", tmp_g2t_DT$region),]
  }
...
#   gene_list <- rownames(manyGenes_rnaseqDT)[i]
#   all_genes <-  unlist(strsplit(gene_list,split="///"))
#   all_regions <- sapply(all_genes, function(x) gene2tadDT$region[gene2tadDT$entrezID == x])
#   # v3: count number of TAD
#   regions_in_TAD <- unlist(lapply(all_regions, function(x) sum(grepl("_TAD", x))))
#   ### v3: take the one that maps to max number of TADs
#   # if none, the one that maps to max number of BD,
#   # if none, the first one
#   if(any(regions_in_TAD > 0)) {
#     select_gene <- all_genes[which.max(regions_in_TAD)]
#     # use any because can map to mutliple TADs and boundaries
#     stopifnot(any(regexpr("_TAD", gene2tadDT$region[gene2tadDT$entrezID == select_gene]) > 0))
#   } else {
#     regions_in_BD <- unlist(lapply(all_regions, function(x) sum(grepl("_BOUND", x))))
#     if(any(regions_in_BD > 0)) {
#       select_gene <- all_genes[which.max(regions_in_BD)]
#       # use any because can map to mutliple TADs and boundaries
#       stopifnot(any(regexpr("_BOUND", gene2tadDT$region[gene2tadDT$entrezID == select_gene]) > 0))
#     } else{
#       select_gene <- all_genes[1]  
#     }
#   }
#   newDT <- manyGenes_rnaseqDT[i,,drop=FALSE]
#   newDT$genes <- select_gene
...
txt <- paste0(toupper(script_name), "> Current threshold for TAD size: >= ", minNbrGeneTAD, " and <= ", upperLimit, " (", maxQuantGeneTAD*100, "%)\n")
printAndLog(txt, pipLogFile)
