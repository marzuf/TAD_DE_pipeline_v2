...
permut_ratioDown_DT <- eval(parse(text = load(paste0(pipOutFold, "/", script8_name, "/ratioDown_permDT.Rdata"))))

### !!! NEED TO CHECK WHY THERE ARE SOME NA
permut_ratioDown_DT <- na.omit(permut_ratioDown_DT)

nRandom <- nRandomPermut

intersectRegions <- intersect(names(obs_ratioDown), rownames(permut_ratioDown_DT))
...
if(useTADonly) {
    if( length(grep("_TAD", rownames(permut_ratioDown_DT))) > 0 ) {
        txt <- paste0(toupper(script_name), "> !!! WARNING: permutation ratioDown data contain non-TAD regions as well !!!\n")
        printAndLog(txt, pipLogFile)    
    }
...
initNrow <- nrow(permut_ratioDown_DT)
permut_ratioDown_DT <- permut_ratioDown_DT[intersectRegions,]
txt <- paste0(toupper(script_name), "> Take only the regions form permut for which genes were retained: ", nrow(permut_ratioDown_DT), "/", initNrow, "\n")
printAndLog(txt, pipLogFile)

initLen <- length(obs_ratioDown)
obs_ratioDown <- obs_ratioDown[intersectRegions]
txt <- paste0(toupper(script_name), "> Take only the regions form observ. for which genes were retained: ", length(obs_ratioDown), "/", initLen, "\n")
printAndLog(txt, pipLogFile)

stopifnot(nrow(permut_ratioDown_DT) == length(obs_ratioDown))
...
nTAD <- sum(regexpr("_TAD",  rownames(permut_ratioDown_DT)) > 0)
cat(paste0("... TADs: ", nTAD, "/", nrow(permut_ratioDown_DT), "\n"))
permut_ratioDown_vect <- as.vector(permut_ratioDown_DT[grep("_TAD", rownames(permut_ratioDown_DT)),])
stopifnot(length(permut_ratioDown_vect) == ncol(permut_ratioDown_DT) * nTAD)

cat(paste0("length(permut_ratioDown_vect) = ", length(permut_ratioDown_vect), "\n"))

cat("> Prepare observed data \n")
nTAD <- sum(regexpr("_TAD",  names(obs_ratioDown)) > 0)
cat(paste0("... TADs: ", nTAD, "/", length(obs_ratioDown), "\n"))
obs_ratioDown <- obs_ratioDown[grep("_TAD", names(obs_ratioDown))]

cat("> Prepare data for plotting \n")

obs_rel_hist <- hist(obs_ratioDown, breaks=seq(0,1,histBreakStep), plot=F)
#plot(obs_rel_hist)

cat(paste0("length(permut_ratioDown_vect) = ", length(permut_ratioDown_vect), "\n"))
cat(paste0("length(permut_ratioDown_vect) = ", length(na.omit(permut_ratioDown_vect)), "\n"))
shuff_rel_hist <-  hist(permut_ratioDown_vect,breaks=seq(0,1,histBreakStep), plot=F)
# plot(shuff_rel_hist)

cat(paste0("ncol(permut_ratioDown_DT) = ", ncol(permut_ratioDown_DT), "\n"))
...
stopifnot(sum(obs_rel_hist$counts) == length(na.omit(obs_ratioDown)))
...
rel_HistValues <- apply(permut_ratioDown_DT[grep("_TAD", rownames(permut_ratioDown_DT)),],
