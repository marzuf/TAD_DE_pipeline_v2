...
##########################################################################################
########################################################################################## CALCULATE OBSERVED FC DOWN
##########################################################################################

cat("... start calculate observed FCdown/TAD\n")
all_obs_FCdown <- get_FCdownByRegion_v2(gene2tadDT, DE_topTable) 
txt <- paste0(toupper(script_name), "> Number of observed ratio down computed: ", length(all_obs_FCdown), "\n")
printAndLog(txt, pipLogFile)
save(all_obs_FCdown, file = paste0(curr_outFold, "/all_obs_FCdown.Rdata"))
cat(paste0("... written: ",  paste0(curr_outFold, "/all_obs_FCdown.Rdata"), "\n"))


cat("... start calculate permutations FCdown/TAD\n")
FCdown_permDT <- get_statFromShuffle_para(DEdt=DE_topTable, shuffData=permutationsDT, stat="FCdown", ncpu=nCpu, TADonly=F)
txt <- paste0(toupper(script_name), "> Number of permutations for which ratio down computed: ", ncol(FCdown_permDT), "\n")
printAndLog(txt, pipLogFile)
txt <- paste0(toupper(script_name), "> Number of regions for which ratio down computed: ", nrow(FCdown_permDT), "\n")
printAndLog(txt, pipLogFile)
save(FCdown_permDT, file = paste0(curr_outFold, "/FCdown_permDT.Rdata"))
cat(paste0("... written: ",  paste0(curr_outFold, "/FCdown_permDT.Rdata"), "\n"))
