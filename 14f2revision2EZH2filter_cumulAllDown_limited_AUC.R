...
# revision2_14f2_splitWave => was adapted to be run separately
# and also can accomodate to have a permutDT which is longer (more rows) than the # of TADs in the observed data
# then, GC: take the same TADs in permDT and obs. -> so no need for that again

#*********************#*********************#*********************#*********************
#********************* REVISION_2: HARD-CODED PATH TO FILES AND FOLDERS

script_name <- "14f2revision2EZH2filter_cumulAllDown_limited_AUC"
...
curr_outFold <- paste0(pipOutFold, "/", paste0(script_name))
...
for(curr_ratio_type in allDown_limited) {
    ...
    stopifnot(TADs_toKeep_histFC %in% interReg)
    
#    interReg_nofilter <- interReg # only used in the 1st version for trial in revision2_14f2_splitWave; not needed anymore
    ...
    # filter_permut_currDown_unsort <- permut_currDown[interReg_nofilter,]   # only used in the 1st version for trial in revision2_14f2_splitWave; not needed anymore
    stopifnot(length(filter_obs_curr_down) == nrow(filter_permut_currDown_unsort)) # true only if not use interReg_nofilter
    ...
    plot_cumsumDiff05(filter_obs_curr_down_half, 
      ...
      # PLOT THE 2ND PLOT  # => cannot be plotted if use interReg_nofilter
       if(plotSeparated){
