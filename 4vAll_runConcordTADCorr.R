...
script_name <- "4vAll_runConcordTADCorr"
...
#*************************************************************** !!! HARD-CODED SETTINGS 
withDiag <- FALSE
txt <- paste0(toupper(script_name), "> Take the diagonal when retrieving TAD correlation values of lower triangle: ", as.character(withDiag), "\n")
printAndLog(txt, pipLogFile)
...
reg=all_regions[1]
all_concordCorr_TAD <- foreach(reg=all_regions) %dopar% {
  ...
  corrValues <- corrMatrix_all[lower.tri(corrMatrix_all, diag = withDiag)]
  ratioMagnitude <- sum(corrValues[corrValues>0], na.rm=TRUE)/sum(abs(corrValues))
  ratioNbr <- length(corrValues[corrValues>0])/length(corrValues)
  # corrConcordance <- (2*ratioMagnitude - 1) * (2*ratioNbr - 1) # FCC: score positively if both neg -> not what I want here
  # corrConcordance <- ratioMagnitude * ratioNbr # FCC: score positively if both neg -> not what I want here
  # corrConcordance
  c(ratioMagnitude=ratioMagnitude, ratioNbr=ratioNbr)
}

cat(paste0("... end intra TAD correlation concordance\n"))

names(all_concordCorr_TAD) <- all_regions
stopifnot(length(all_concordCorr_TAD) == length(all_regions))

outFile <- file.path(curr_outFold, "all_concordCorr_TAD.Rdata")
save(all_concordCorr_TAD, file= outFile)
