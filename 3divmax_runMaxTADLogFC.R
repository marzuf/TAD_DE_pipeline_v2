...
script_name <- "3divmax_runMaxTADLogFC"
...
mean_DT <- aggregate(logFC ~ region, data=mergedDT, FUN=function(x) {

i_max <- which.max(abs(x))
...
save(all_meanLogFC_TAD, file= paste0(curr_outFold, "/all_divmaxLogFC_TAD.Rdata"))
cat(paste0("... written: ", curr_outFold, "/all_divmaxLogFC_TAD.Rdata", "\n"))
