...
script_name <- "4partial_runMeanTADCorr"
...
suppressPackageStartupMessages(library(psych, warn.conflicts = FALSE, quietly = TRUE, verbose = FALSE)) # for partial correlation
...
# NB 02.07.2019 -> this is ok because I have reordered norm_rnaseqDT : norm_rnaseqDT <- norm_rnaseqDT[names(geneList),]    
  ...
  stopifnot(ncol(subData) == length(reg_genes))
  #### CORRELATION
  full_corrMatrix_all <- cor(subData, method = corrMethod)
  
  #### CORRELATION ===> change here for partial correlation (26.11.2019)
  fpkmdt <- t(subData)
  stopifnot(colnames(fpkmdt) %in% names(curr_purity))
  fpkmdt_with_pur <- as.data.frame(fpkmdt)
  fpkmdt_with_pur["purity",] <- curr_purity[colnames(fpkmdt_with_pur)]
  stopifnot(rownames(fpkmdt_with_pur)[nrow(fpkmdt_with_pur)] == "purity")
    ...
    stopifnot(rownames(partialcorr) == rownames(full_corrMatrix_all))
    stopifnot(colnames(partialcorr) == colnames(full_corrMatrix_all))
    
    
    corrMatrix_all <- partialcorr
