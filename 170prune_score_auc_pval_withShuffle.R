...
script_name <- "170prune_score_auc_pval_withShuffle"
...
#all_scores_DT <- foreach(pipOutFold = comp_folders, .combine = 'rbind') %do% {
  ...
  allratio_prune_thresh <- foreach(curr_ratio_type = allDown, .combine = 'c') %do% {
    ...
    while( (i_permGenes == 0 | i_shuffTADs == 0) & i <= length(curr_filter_obs_curr_down) ) {
      ...
      if(i_shuffTADs == 0) {
        auc_ratio_stat_dep05_shuffTADs <- calc_aucObsPerm_cumsum_list(observ_vect = curr_filter_obs_curr_down[i:length(curr_filter_obs_curr_down)],
                                                                      shuff_List=lapply(curr_shuff_currDown, function(x) {
                                                                        end_pos <- max(c(1, length(x) -i +1))
                                                                        x[1:end_pos]}),
                                                                      # observ_vect = curr_filter_obs_curr_down[i:length(curr_filter_obs_curr_down)],
                                                                      # shuff_List=lapply(curr_shuff_currDown, function(x) x[i:length(x)]),
                                                                      # observ_vect = curr_filter_obs_curr_down,
        ...
        if(auc_ratio_stat_dep05_shuffTADs < 1) {
          i_shuffTADs <- i
          ratio_pruneThresh_shuffTADs <- curr_filter_obs_curr_down[i]
          auc_ratio_pruneThresh_shuffTADs <- auc_ratio_stat_dep05_shuffTADs
          cat(paste0("----- ", curr_ratio_type, " - break shuffTADs: i=", i_shuffTADs, " (ratio=", round(ratio_pruneThresh_shuffTADs,2), ")\t found AUC=", auc_ratio_stat_dep05_shuffTADs, "\n"))
        } else{
          cat("...... ", curr_ratio_type, " - shuffTADs AUC = ", auc_ratio_stat_dep05_shuffTADs, "\t -> go to next iteration\n")
        }
      }
    ...
    foo1 <- calc_aucObsPerm_cumsum_list(observ_vect = curr_filter_obs_curr_down,
                                                                  shuff_List=curr_shuff_currDown,
                                                                  thresh_perm =permThresh,
                                                                  doPlot=TRUE,
                                                                  departureValue=departFromValue)
    
    foo1 <- calc_aucObsPerm_cumsum_list(observ_vect = curr_filter_obs_curr_down[i_shuffTADs:length(curr_filter_obs_curr_down)],
                                                                  # shuff_List=lapply(curr_shuff_currDown, function(x) x[i_shuffTADs:length(x)]),
                                        shuff_List=lapply(curr_shuff_currDown, function(x) {
                                          end_pos <- max(c(1, length(x) -i_shuffTADs +1))
                                          x[1:end_pos]}),
                                                                  thresh_perm =permThresh,
                                                                  doPlot=TRUE,
                                                                  departureValue=departFromValue)
  ...
  names(allratio_prune_thresh) <- paste0(rep(allDown,each=6), c("_idx_prune_permGenes", "_ratio_prune_permGenes", "_auc_prune_permGenes", 
                                                                "_idx_prune_shuffTADs", "_ratio_prune_shuffTADs", "_auc_prune_shuffTADs"))



outFile <-  paste0(curr_outFold, "/", "allratio_prune_thresh.Rdata")
save(allratio_prune_thresh, file = outFile)
cat(paste0("... written: ", outFile, "\n"))
