...
stopifnot(names(pipeline_geneList) %in% rownames(fpkm_rnaseqDT))
fpkm_rnaseqDT <- fpkm_rnaseqDT[names(pipeline_geneList),]    
stopifnot(all(rownames(fpkm_rnaseqDT) == names(pipeline_geneList)))
stopifnot(!any(duplicated(names(pipeline_geneList))))
stopifnot(!any(duplicated(rownames(fpkm_rnaseqDT))))
stopifnot(!any(duplicated(names(pipeline_geneList))))
...
fpkm_rnaseqDT <- fpkm_rnaseqDT[rownames(fpkm_rnaseqDT) %in% names(pipeline_geneList), ]
...
stopifnot(names(rowMeans_samp1) %in% rownames(fpkm_rnaseqDT))

stopifnot(setequal(rownames(permutationsDT), pipeline_geneList))
stopifnot(setequal(rownames(fpkm_rnaseqDT), names(pipeline_geneList)))
...
# wilcoxStat_permDT <- foreach(i_col = 1:ncol(permutationsDT), .combine='cbind') %dopar% {
  ...
  wilcox_pairedTAD_meanExpr_fpkm <- foreach(i_reg = c(1:length(curr_regions)), .combine='cbind') %do% {
    ...
    reg_genes_avgExpr_cond1b <- rowMeans_samp1[perm_genes_fpkm]
    reg_genes_avgExpr_cond2b <- rowMeans_samp2[perm_genes_fpkm]
    stopifnot(!is.na(reg_genes_avgExpr_cond1b))
    stopifnot(!is.na(reg_genes_avgExpr_cond2b))
    stopifnot(names(reg_genes_avgExpr_cond1b) == names(reg_genes_avgExpr_cond2b) )
    
    wTest2 <- my_wilcox_onlyW.test(reg_genes_avgExpr_cond1b, reg_genes_avgExpr_cond2b, 
                                  paired=wilcoxPaired, alternative=wilcoxAlternative,
                                  exact=FALSE) # don't need for the pval
