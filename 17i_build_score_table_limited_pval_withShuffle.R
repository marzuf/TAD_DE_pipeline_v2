...
script_name <- "17i_build_score_table_limited_pval_withShuffle"
...
all_scores_DT <- foreach(curr_ds = comp_folders, .combine = 'rbind') %do% {
  ...
  allratio_auc <- foreach(curr_ratio_type = allDown, .combine = 'c') %do% {
                  ...
                  file3 <- paste0(curr_ds, "/", script8_name, "/", curr_ratio_type, "_randomShuffleList.Rdata")
                 stopifnot(file.exists(file1), file.exists(file2), file.exists(file3))
#                return(NULL)
