...
script_name <- "3max_runMaxTADLogFC"
...
mean_DT <- aggregate(logFC ~ region, data=mergedDT, FUN=function(x) max(abs(x)))
...
save(all_meanLogFC_TAD, file= paste0(curr_outFold, "/all_maxLogFC_TAD.Rdata"))
cat(paste0("... written: ", curr_outFold, "/all_maxLogFC_TAD.Rdata", "\n"))
