...
script_name <- "14f2split_cumulAllDown_limited_AUC"
...
for(curr_ratio_type in allDown_limited) {
    ...
    # PLOT THE 1ST PLOT
      if(plotSeparated) {
        outFile <- paste0(curr_outFold, "/", curr_ratio_type, "_departure05_cumsum_obs_permut_withRatio_fullCurve.", plotType)
      ...
      if(plotSeparated){
        outFile <- paste0(curr_outFold, "/", curr_ratio_type, "_departure05_cumsum_obs_permut_withRatio_1stPart.", plotType)
        do.call(plotType, list(outFile, height=myHeight, width=myWidth))
      }
