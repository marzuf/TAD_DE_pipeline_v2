...
script_name <- "9sameNbrDouble_runEmpPvalMeanTADLogFC"
...
fc_typeA <- "meanLogFC_right" #  "meanLogFC", "meanLogFC_right", "meanLogFC_left"
fc_typeB <- "meanLogFC_left" #  "meanLogFC", "meanLogFC_right", "meanLogFC_left"
...
printAndLog(txt, pipLogFile)
txt <- paste0("settingF\t=\t", fc_typeA, " - ", fc_typeB, "\n")
printAndLog(txt, pipLogFile)
...
all_sample_fcValues <- foreach(fc_file = all_sampleFC_files, .combine='c') %dopar% {
  ...
  all_samp_fcs1 <- as.numeric(sapply(fc_data, function(x) x[[paste0(fc_typeA)]]))
  all_samp_fcs2 <- as.numeric(sapply(fc_data, function(x) x[[paste0(fc_typeB)]]))
	all_samp_fcs <- c(all_samp_fcs1, all_samp_fcs2)
