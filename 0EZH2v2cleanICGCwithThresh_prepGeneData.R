...
# - rna_geneList.Rdata
# - rna_rnaseqDT.Rdata
# - rna_madnorm_rnaseqDT.Rdata # NOT FOR ICGC
# UPDATE 20.06.2018 - for ICGC, no madnorm but qqnorm
# - rna_qqnorm_rnaseqDT.Rdata # AS ICGC IS RNAseq
## UPDATE 20.06.2018
# - pipeline_geneList.Rdata    # NOT OUTPUTED ANYMORE, IS OUTPUTED IN SCRIPT1 RUN DE
# - pipeline_regionList.Rdata  # NOT OUTPUTED ANYMORE, IS OUTPUTED IN SCRIPT1 RUN DE
# - pipeline_geneList_nofilter.Rdata
# - pipeline_regionList_nofilter.Rdata
# (+ some pictures)
...
# => 19.06.2018
# => there is no // to split in the rownames
# => in the 0EZH2clean version, there is an error in the foreach loop, 1:dim(...) =>but won't be ok, if no pattern...

# => 20.06 
# there are RNAseq data !!! check that should correspond to the RNAseq implementation from 0cleanInput
...
if(! normRNAseq) {
  stop("script for RNAseq data from ICGC data\n")
}
...
# ADDED 20.06.2018 # loaded from settingF
...
if(is.null(gene_adjPvalThresh) & is.null(gene_absLogfcThresh) ) stop("Both gene_pvalThresh and gene_logfcThresh => meaningless !!!\n")
...
# UPDATE 19.06.2018 FOR ICGC DATA, THE 1:dim() WON'T BE OK IF 0 !!!

if(nrow(manyGenes_rnaseqDT) > 0) {
	...
	stopifnot(nrow(manyGenes_rnaseqDT) == 0)
	all_DT <- singleGene_rnaseqDT
}

#save(all_DT, file = "/mnt/ed4/marie/scripts/EZH2_final_MAPQ/all_DT1.Rdata")
#cat("written: /mnt/ed4/marie/scripts/EZH2_final_MAPQ/all_DT1.Rdata\n")
...
all_DT$rowNbr <- NULL
#save(all_DT, file = "/mnt/ed4/marie/scripts/EZH2_final_MAPQ/all_DT2.Rdata")
#cat("written: /mnt/ed4/marie/scripts/EZH2_final_MAPQ/all_DT2.Rdata\n")

# 19.06.2018: UPDATE FOR ICGC
#stopifnot(!any(duplicated(all_DT))) # is not True for ICGC !!! warning:genes with all zeros ??!!
tmp_ICGC <- all_DT
tmp_ICGC$rowN <- rownames(tmp_ICGC)
stopifnot(!any(duplicated(tmp_ICGC)))
...
### UPDATE 20.06.2018: for RNAseq from ICGCs, need FPKM normalization
if(normRNAseq){
...
if(microarray) {
  # UPDATE 20.06.2018: SHOULD NOT HAPPEN FOR ICGC
  stop("SHOULD NOT HAPPEN\n")
...
if(useFilterCountData) {
  ...
  # ensure the samples are present in the column names
	head(colnames(countFilter_rnaseqDT))
	head(samp1)
	head(samp2)
...
printAndLog(txt, pipLogFile)
## CHANGED 20.06.2018 for the filter !!!
...
pipeline_geneList_nofilter <- pipeline_geneList
save(pipeline_geneList_nofilter, file = paste0(curr_outFold, "/", "pipeline_geneList_nofilter.Rdata"))
cat(paste0("... written: ", paste0(curr_outFold, "/", "pipeline_geneList_nofilter.Rdata"), "\n"))
...
pipeline_regionList_nofilter <- pipeline_regionList
save(pipeline_regionList_nofilter, file = paste0(curr_outFold, "/", "pipeline_regionList_nofilter.Rdata"))
cat(paste0("... written: ", paste0(curr_outFold, "/", "pipeline_regionList_nofilter.Rdata"), "\n"))
