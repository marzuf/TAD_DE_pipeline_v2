...
script6_name <- "6pF_runPermutationsMeanLogFC" # <<< 19.08.20: add purity filter !!!
script_name <- "9pF_runEmpPvalMeanTADLogFC" # <<< 19.08.20: add purity filter !!!
...
obs_TADLogFC <- obs_TADLogFC[names(obs_TADLogFC) %in% basename(ds_TADs_to_keep)]
stopifnot(length(obs_TADLogFC) > 0)
...
#>><<< 19.08.20: add purity filter !!! for check pf
stopifnot(length(obs_TADLogFC) == nrow(permut_TADLogFC_DT))
