...
script_name <- "14i2splitSlope_cumulAllDown_limited_AUC_randomTADsShuffle"
...
for(curr_ratio_type in allDown_limited) {
  ...
  if(plotSeparated){
    outFile <- paste0(curr_outFold, "/", curr_ratio_type, "_departure", departureFromValue,"_deviation_obs_permut_withRatio_fullCurve.", plotType)
  ...
  onlyOne_sort_obs_curr_down <- sort_obs_curr_down[sort_obs_curr_down == 1]
  ...
  stopifnot(sort_obs_curr_down[1:length(onlyOne_observ_vect)] == 1)
    ...
    if(length(onlyOne_observ_vect) > 0 ) {
