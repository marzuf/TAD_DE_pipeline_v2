...
script5_name <- "5pF_runPermutationsMedian"  # >><<< 19.08.20: change here for purity filter !!!
script_name <- "6pF_runPermutationsMeanLogFC" ### same outfolder as non random
# stopifnot(file.exists(paste0(pipScriptDir, "/", script_name, ".R"))) # not true shorter script name in fact...
...
#####>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<>>>>>>>><<< 19.08.20: add purity filter !!! 24.08 ensure random data
curr_exprds <- basename(pipOutFold)
curr_hicds <- basename(dirname(pipOutFold))

stopifnot(grepl("RANDOM" , curr_hicds))


purityFilterFile <- file.path("//mnt/etemp/marie/v2_Yuanlong_Cancer_HiC_data_TAD_DA_PURITYFILTER/PREP_DATA_PURITYFILTER_RANDOM/aran/CPE/log10/TADs_to_keep.Rdata")
...
stopifnot(init_nGenes == length(pipeline_geneList))
...
rowsToKeep <- which(pipeline_geneList %in% as.character(pF_gene2tadDT$entrezID))
pipeline_geneList <- pipeline_geneList[rowsToKeep]

stopifnot(as.character(pipeline_geneList) %in% gene2tadDT$entrezID)
gene2tadDT <- gene2tadDT[gene2tadDT$entrezID %in% as.character(pipeline_geneList),]

stopifnot(gene2tadDT$region %in% basename(ds_TADs_to_keep))
stopifnot(length(gene2tadDT$entrezID) == length(pipeline_geneList))
...
#####>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<>>>>>>>><<< 19.08.20: add purity filter !!!
pipeline_regionList <- pipeline_regionList[pipeline_regionList %in% basename(ds_TADs_to_keep)]
stopifnot(setequal(pipeline_regionList,gene2tadDT$region ))
stopifnot(length(unique(permutationsDT[,1])) == length(pipeline_regionList))
#####>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<>>>>>>>><<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
