...
script_name <- "test5_runPermutationsMedian"
...
source(paste0(pipScriptDir, "/", "TAD_DE_shuffle.R"))
...
if(withExprClass) {
    shuffleData <- get_multiShuffledPositions_vFunct_v1(g2TADdt=gene2tadDT, RNAdt=rnaseqDT, 
...
cat("... Start permutations\n")
if(withExprClass) {
  shuffleData <- get_multiShuffledPositions_vFunct_v2(g2TADdt=gene2tadDT, RNAdt=rnaseqDT, 
                                                   geneIDlist=geneList, nClass = nClass, TADonly=F, nSimu=nRandom, 
                                                   withExprClass=withExprClass, nCpu=nCpu, aggregFun = aggregFction)
} else {
  shuffleData <- get_multiShuffledPositions_vFunct_v2(g2TADdt=gene2tadDT, RNAdt=rnaseqDT, 
...
save(permutationsDT, file = paste0(curr_outFold, "/permutationsDT_v2.Rdata"))
cat(paste0("... written: ", paste0(curr_outFold, "/permutationsDT_v2.Rdata"), "\n"))
...
cat(paste0("nRandom = ", nRandom,  "\n"))
cat(paste0("withExprClass = ", as.character(withExprClass),  "\n"))
cat(paste0("... start1 = ", start1, "\n"))
cat(paste0("... end1 = ", end1, "\n"))
cat(paste0("... start2 = ", start2, "\n"))
cat(paste0("... end2 = ", end2, "\n"))