...
script_name <- "14f3revision2EZH2filter_cumulAllDown_withDist_AUC"
...
if(! plotSeparated) {
  ...
  outFile <- paste0(curr_outFold, "/allRatios_cumsum_obs_permut_withDist.", plotType)
...
for(curr_ratio_type in allDown_limited) {
    ...
    # PLOT THE 1ST PLOT (cumsum with polygon area)
      if(plotSeparated) {
    ...
    # use plot_cumsumDiff05_revision2 if permut and observed do not have same number of TADs
      ...
      # PLOT THE 2ND PLOT (distance between the 2 curves)
      if(plotSeparated){
        outFile <- paste0(curr_outFold, "/", curr_ratio_type, "_departure05_cumsum_obs_permut_distPermut.", plotType)
        do.call(plotType, list(outFile, height=myHeight, width=myWidth))
      }
