...
script_name <- "14i2_cumulAllDown_limited_AUC_randomTADsShuffle"
...
lineMinCol <- rgb(0/255,16/255,200/255)
lineMaxCol <- rgb(180/255,16/255,0/255)
# plot(NULL, xlim = range(1:10), ylim= range(1:10))
# lines(x=1:10, y=1:10, col = lineMin, type="p", pch=16)
# lines(x=1:10, y=1:10, col = lineMax, lty=2)
...
for(curr_ratio_type in allDown_limited) {
  ...
  if(step14_for_randomTADsShuffle){
    ...
    # to control the suffling, plot the line to check why I dont have polygon area
    lines(x=1:maxElements, y=minLine, col = lineMinCol, type="p", pch=16)
    lines(x=1:maxElements, y=maxLine, col = lineMaxCol, lty=2)
  }
  ...
  ### PLOT the second plot (distribution with vertical line)
  # PLOT THE 2ND PLOT
  if(plotSeparated){
  ...
  if(step14_for_randomTADsShuffle){
    ...
    stopifnot(length(all_random_List[["shuffle"]]) == nRandomPermutShuffle)
    
    plot_cumsumDiff05_AUC2_list(observ_vect_cumsum=observ_vect, 
                                permut_List_cumsum =all_random_List[["shuffle"]], 
