...
script_name <- "14f2splitSlope_cumulAllDown_limited_AUC"
...
for(curr_ratio_type in allDown_limited) {
    ...
    # PLOT THE 1ST PLOT
      if(plotSeparated) {
        outFile <- paste0(curr_outFold, "/", curr_ratio_type, "_departure05_deviation_obs_permut_withRatio_fullCurve.", plotType)
        do.call(plotType, list(outFile, height=myHeight, width=myWidth))
      }
      stopifnot(length(filter_obs_curr_down_half) == nrow(filter_permut_currDown_half) )
      ...
      # add the AUC ratio
      auc_ratio <- calc_aucObsPerm_cumsum(observ_vect = filter_obs_curr_down_half,
      ...
      all_ratio_split <- append(all_ratio_split, setNames(auc_ratio, paste0(curr_ratio_type, "_auc_permGenes_all")))
      ...
      if(plotSeparated){
        outFile <- paste0(curr_outFold, "/", curr_ratio_type, "_departure05_deviation_obs_permut_withRatio_1stPart.", plotType)
        do.call(plotType, list(outFile, height=myHeight, width=myWidth))
      }
      
      # PLOT THE 2ND PLOT
      # => plot the first part of the curve where the observed data == 1
      toRemove <- which(filter_obs_curr_down_half <  1)
      onlyOne_filter_obs_curr_down_half <- filter_obs_curr_down_half[-as.numeric(toRemove)]
      stopifnot(onlyOne_filter_obs_curr_down_half == 1)
      onlyOne_filter_permut_currDown_half <- filter_permut_currDown_half[-as.numeric(toRemove),]
      stopifnot(length(onlyOne_filter_obs_curr_down_half) == nrow(onlyOne_filter_permut_currDown_half) )      
      
      # plot_cumsumDiff05(onlyOne_filter_obs_curr_down_half, 
      #                   onlyOne_filter_permut_currDown_half, 
      #                   my_stat = my_stat_curr_ratio,
      #                   departureValue = departureFromValue, drawline=TRUE)
      # mtext(subtitDir, font=3)
      # add the AUC ratio
      auc_ratio_onlyOne <- calc_aucObsPerm_cumsum(observ_vect = onlyOne_filter_obs_curr_down_half,
                                          permut_DT=onlyOne_filter_permut_currDown_half,
                                          my_stat = curr_ratio_type,
                                          thresh_perm =permThresh,
                                          doPlot=F,
                                          departureValue=departureFromValue,
      ...
      all_ratio_split <- append(all_ratio_split, setNames(auc_ratio_onlyOne, paste0(curr_ratio_type, "_auc_permGenes_one")))
      ...
      if(plotSeparated){
        outFile <- paste0(curr_outFold, "/", curr_ratio_type, "_departure05_deviation_obs_permut_withRatio_2ndPart.", plotType)
        do.call(plotType, list(outFile, height=myHeight, width=myWidth))
      }
      
      # PLOT THE 3D PLOT
      # => plot the second part of the curve where the observed data > 1
      oneToRemove <- which(filter_obs_curr_down_half ==  1)
      noOne_filter_obs_curr_down_half <- filter_obs_curr_down_half[-as.numeric(oneToRemove)]
      stopifnot(noOne_filter_obs_curr_down_half < 1)
      noOne_filter_permut_currDown_half <- filter_permut_currDown_half[-as.numeric(oneToRemove),]
      stopifnot(length(noOne_filter_obs_curr_down_half) == nrow(noOne_filter_permut_currDown_half) )      
      
#                                  plot_cumsumDiff05(noOne_filter_obs_curr_down_half, 
#                                                    noOne_filter_permut_currDown_half, 
#                                                    my_stat = my_stat_curr_ratio,
#                                                    departureValue = departureFromValue, drawline=TRUE)
#                                  mtext(subtitDir, font=3)
      # add the AUC ratio
      auc_ratio_noOne <- calc_aucObsPerm_cumsum(observ_vect = noOne_filter_obs_curr_down_half,
                                                  permut_DT=noOne_filter_permut_currDown_half,
                                                  my_stat = curr_ratio_type,
                                                  thresh_perm =permThresh,
                                                  doPlot=F,
                                                  departureValue=departureFromValue,
      ...
      all_ratio_split <- append(all_ratio_split, setNames(auc_ratio_noOne, paste0(curr_ratio_type, "_auc_permGenes_2ndPart")))
