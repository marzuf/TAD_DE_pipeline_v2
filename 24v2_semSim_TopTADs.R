...
script_name <- "24v2_semSim_TopTADs"
...
all_sizes_random_semSim <- lapply(topTADs_size, function(x) {
  ...
  while(length(all_random_semSim) < nRandomSemSim) {
    ...
    missing_random_semSim <- foreach(curr_perm = seq_len(stillMissing), .combine='c') %dopar% { 
      
      cat(paste0("... for topTADs_size: ", x, " - curr_perm = ", curr_perm, "/", stillMissing, "\n"))
      
      # try(unloadNamespace("GOSemSim"))
      # try(unloadNamespace("org.Hs.eg.db"))
      # try(unloadNamespace("GO.db"))
      # try(unloadNamespace("AnnotationDbi"))
      ...
      random_semSim <- mgeneSim(genes=randomGenes,
                                # semData=hsGO,
                                semData=godata('org.Hs.eg.db', ont="BP", computeIC = semSimMetric!="Wang"),
...
# topTADs_semSim <- lapply(names(topTADs_genes), function(x) {
  topTADs_semSim <- foreach(x=names(topTADs_genes)) %dopar% {
