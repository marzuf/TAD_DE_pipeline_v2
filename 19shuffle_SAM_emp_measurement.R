...
script_name <- "19shuffle_SAM_emp_measurement"
...
shuff_ratioDown_file <- file.path(pipOutFold, script8_name, "ratioDown_randomShuffleList.Rdata")
...
shuff_prodSignedRatio_file <- file.path(pipOutFold, script8_name, "prodSignedRatio_randomShuffleList.Rdata")
...
stopifnot(setequal(names(obs_vect_prodSignedRatio), names(obs_vect_ratioDown)))
...
commonReg <- sort(names(obs_vect_prodSignedRatio))
...
permutDT <- lapply(permutDT, function(x) x - 0.5)
...
# symmetric: observ_N <- sum(abs(obs_vect) >= abs(cut_off))
empFDR_seq <- unlist(sapply(cut_off_seq, function(x) get_SAM_FDR(obs_vect, permutDT, cut_off = x, symDir = "symmetric", withPlot = F, inputList=T)))
...
# PLOT ALL VALUES AND AREA PERMUT WITH FDR for the given cut-off
# this I cannot draw for the shuffle because the TADs do not remain
# k_cut_off <- 0.3
# outFile <- paste0(curr_outFold, "/", "FDR_", k_cut_off, "_cut_off_", curr_variable, ".", plotType)
# do.call(plotType, list(outFile, height=myHeight, width=myWidth))
# get_SAM_FDR(obs_vect, permutDT, cut_off = k_cut_off, symDir = "symmetric",  variableName = curr_variable, withPlot = TRUE, plotOffsetY = 0.05, inputList = T)
# textTAD <- names(obs_vect)[which(obs_vect >= k_cut_off  | obs_vect <= -k_cut_off ) ] # symmetric !
# if(length(textTAD) > 0)
#   text(y=obs_vect[textTAD], x = which(names(obs_vect) %in% textTAD), labels = textTAD, pos=2, col="gray")
# cat(paste0("... written: ", outFile, "\n"))
...
# higher: sum(obs_vect >= cut_off)
empFDR_seq <- unlist(sapply(cut_off_seq, function(x) get_SAM_FDR(obs_vect, permutDT, cut_off = x, symDir = "higher", withPlot = F, inputList = T)))
...
# outFile <- paste0(curr_outFold, "/", "FDR_", k_cut_off, "_cut_off_", curr_variable, ".", plotType)
# do.call(plotType, list(outFile, height=myHeight, width=myWidth))
# get_SAM_FDR(obs_vect, permutDT, cut_off = k_cut_off, symDir = "higher",  variableName = curr_variable, withPlot = TRUE, plotOffsetY = 0.05, inputList = T)
# textTAD <- names(obs_vect)[which(obs_vect >= k_cut_off  ) ] # higher !
# if(length(textTAD) > 0)
#   text(y=obs_vect[textTAD], x = which(names(obs_vect) %in% textTAD), labels = textTAD, pos=2, col="gray")
# cat(paste0("... written: ", outFile, "\n"))
