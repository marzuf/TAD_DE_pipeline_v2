...
script_name <- "9randomRescaled_runEmpPvalMeanTADLogFC"
...
txt <- paste0("!!! take all the files matching \"all_meanLogFC_TAD.Rdata\" in ", mainPipFold, "\n")
printAndLog(txt, pipLogFile)


#/mnt/etemp/marie/v2_Yuanlong_Cancer_HiC_data_TAD_DA/PIPELINE/OUTPUT_FOLDER/ENCSR489OCU_NCI-H460_40kb/TCGAluad_norm_luad//3_runMeanTADLogFC/all_meanLogFC_TAD.Rdata 
all_sampleFC_files <- list.files(mainPipFold, pattern="all_meanLogFC_TAD.Rdata", full.names = TRUE, recursive = TRUE)
stopifnot(length(all_sampleFC_files ) > 0)
all_sampleFC_files <- all_sampleFC_files[grepl(randomPattern, all_sampleFC_files) & grepl(script3_name, all_sampleFC_files)]
...
all_sample_fcValues <- foreach(fc_file = all_sampleFC_files, .combine='c') %dopar% {
  ...
  all_samp_fcs <- as.numeric(fc_data)

	# change 21.03.2020 here to rescale:
