...
script_name <- "23vAll_funcEnrich_TopTableGenes_TADsGenes"
...
# do not parallelize doMC but launch all script in para
# library(foreach)
# library(doMC)
...
nTopGO <- length(adj_pval_combined)
nTopKEGG <- length(adj_pval_combined)
...
topTADs <- names(adj_pval_combined)
...
topTADs_Fisher <- names(adj_pval_combined)
nTopTADs_Fisher <- length(topTADs_Fisher)
...
topTADs_GSEA <- GSEA(geneList= score_vect,
                     TERM2GENE=term2gene,
                     nPerm=nPerm_GSEA,
                     pvalueCutoff=1,
                     pAdjustMethod= "BH",
...
# do an enrichGO for each TAD separately instead of having pooled all the genes from the topTADs together

sepTAD_genes_enrichGO <- foreach(curr_tad = topTADs) %dopar% {
  
  currTAD_genes <- as.character(curr_g2t_DT$entrezID[curr_g2t_DT$region == currTAD])
  currTAD_genes_symb <- as.character(unlist(sapply(currTAD_genes, function(x) names(geneList[geneList == x]))))
  
  
  
  ### FOR THE currTAD_genes
  currTAD_genes_scoreDT <- curr_topTable_DT[curr_topTable_DT$genes %in% currTAD_genes_symb, c("genes","adj.P.Val", "entrezID")]
  
  # for the moment, background genes set to universe
  if("keytype" %in% formalArgs(enrichGO)) {
    currTAD_genes_enrichGO <- enrichGO(gene = currTAD_genes_scoreDT$entrezID,
                                       OrgDb = org.Hs.eg.db,
  ...
  currTAD_genes_enrichGO_resultDT <- currTAD_genes_enrichGO@result
  currTAD_genes_enrichGO_resultDT <- currTAD_genes_enrichGO_resultDT[order(currTAD_genes_enrichGO_resultDT$p.adjust, currTAD_genes_enrichGO_resultDT$pvalue),]
  currTAD_genes_enrichGO_resultDT
} 

outFile <- paste0(curr_outFold, "/", "sepTAD_genes_enrichGO.Rdata")
save(sepTAD_genes_enrichGO, file = outFile)
cat(paste0("... written: ", outFile, "\n"))
...
sepTAD_genes_enrichKEGG <- foreach(curr_tad = topTADs) %dopar% {
  ...
  # for the moment, background genes set to universe
  currTAD_genes_enrichKEGG <- enrichKEGG(gene = currTAD_genes_scoreDT$entrezID,
                                         organism = "hsa",
                                         keyType = "ncbi-geneid",
                                         pvalueCutoff = 1,
                                         minGSSize = 1,
  ...
  currTAD_genes_enrichKEGG_resultDT <- currTAD_genes_enrichKEGG@result
  currTAD_genes_enrichKEGG_resultDT <- currTAD_genes_enrichKEGG_resultDT[order(currTAD_genes_enrichKEGG_resultDT$p.adjust, currTAD_genes_enrichKEGG_resultDT$pvalue),]
  currTAD_genes_enrichKEGG_resultDT
} 

outFile <- paste0(curr_outFold, "/", "sepTAD_genes_enrichKEGG.Rdata")
save(sepTAD_genes_enrichKEGG, file = outFile)
cat(paste0("... written: ", outFile, "\n"))
