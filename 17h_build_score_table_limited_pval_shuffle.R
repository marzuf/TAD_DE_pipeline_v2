...
script_name <- "17h_build_score_table_limited_pval_shuffle"
...
all_scores_DT <- foreach(curr_ds = comp_folders, .combine = 'rbind') %do% {
  ...
  allratio_auc <- foreach(curr_ratio_type = allDown, .combine = 'c') %do% {
    ...
    cat(paste0("... load: ", paste0(curr_ds, "/", script8_name, "/", curr_ratio_type, "__randomShuffleList.Rdata"), "\n"))
    shuff_currDown <-  eval(parse(text = load(paste0(curr_ds, "/", script8_name, "/", curr_ratio_type, "_randomShuffleList.Rdata"))))
    ...
    auc_ratio_stat_dep05_shuffTADs <- calc_aucObsPerm_cumsum_list(observ_vect = curr_filter_obs_curr_down,
    ...
    c(auc_ratio_stat_dep05_shuffTADs, auc_ratio_stat_dep05_pval_shuffTADs)
...
dtFile <- paste0(curr_outFold, "/rank_all_scores_DT_withPval_shuff.txt")
...
all_scores_samples_DT <- all_scores_samples_DT[,c("dataset", "process", "cond1", "cond2", "nSamp1", "nSamp2",
                                                  ...
                                                  tmpCol)]
dtFile <- paste0(curr_outFold, "/rank_all_scores_DT_withSamp_withPval_shuff.txt")
write.table(all_scores_samples_DT, file = dtFile, row.names=F, col.names=T, quote=F, sep="\t")
cat(paste0("... written: ", dtFile, "\n"))

plotFile <- file.path(curr_outFold, "rank_all_scores_DT_withPval_shuff.svg")
...
plotFile <- file.path(curr_outFold, "rank_all_scores_DT_withSamp_withPval_shuff.svg")
