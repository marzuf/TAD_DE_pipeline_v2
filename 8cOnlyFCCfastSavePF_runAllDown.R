...
# purityFilter version -> use purityFilter data in input = keep only the TADs that are TADs_to_keep from prep_data_purityFilter.R (cf. lines with 19.08.20 additions)
...
script5_name <- "5pF_runPermutationsMedian"
script_name <- "8cOnlyFCCpF_runAllDown"
#stopifnot(file.exists(paste0(pipScriptDir, "/", script_name, ".R"))) not possible for pF
...
#    cat(paste0("... written: ",  paste0(curr_outFold, "/", curr_ratio_type, "_permDT.Rdata"), "\n"))
    my_save.pigz(ratio_permDT,  pigz_exec_path = pigz_exec_path, file = paste0(curr_outFold, "/", curr_ratio_type, "_permDT.Rdata"))
    cat(paste0("... written: ",  paste0(curr_outFold, "/", curr_ratio_type, "_permDT.Rdata"), "\n"))
