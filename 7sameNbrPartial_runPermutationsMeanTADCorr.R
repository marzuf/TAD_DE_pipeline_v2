...
### !!! HARD CODED na use partial corr
### !!! HARD CODED path to purity file
...
script_name <- "7sameNbrPartial_runPermutationsMeanTADCorr"
...
suppressPackageStartupMessages(library(psych, warn.conflicts = FALSE, quietly = TRUE, verbose = FALSE)) # for partial correlations
...
### ADDED HERE HARD-CODED PURITY FILE
purity_file <- file.path(setDir, "/mnt/etemp/marie/v2_Yuanlong_Cancer_HiC_data_TAD_DA/EPIC_PURITY/all_purity_values.Rdata")
stopifnot(file.exists(purity_file))
txt <- paste0("purity_file\t=\t", purity_file, "\n")
printAndLog(txt, pipLogFile)
purity_data <- get(load(purity_file))
curr_exprds <- basename(pipOutFold)
curr_hicds <- basename(dirname(pipOutFold))
stopifnot(file.path(curr_hicds, curr_exprds) %in% names(purity_data))
curr_purity <- purity_data[[file.path(curr_hicds, curr_exprds)]]
stopifnot(length(curr_purity) > 0)

# add here for partial correlation (equivalent to na.rm=true)
cor_na_method <- "pairwise.complete.obs"
txt <- paste0("cor_na_method\t=\t", cor_na_method, "\n")
printAndLog(txt, pipLogFile)
...
meanCorr_sample_around_TADs_sameNbr <- foreach(reg = all_regs) %dopar% {
  ...
  if(tad_data$nGenes > 0) {
    ...
    meanCorr_all <- get_meanCorrPartial_value(
                   ...
                   cormet = corrMeth,
                   purity_vals=curr_purity, 
                   nause=cor_na_method
                   )
    meanCorr_cond1 <- get_meanCorrPartial_value(
    ...
    meanCorr_cond2 <- get_meanCorrPartial_value(
    ...
    if(tad_data$nGenes_right > 0) {
      ...
      meanCorrRight_all <- get_meanCorrPartial_value(
      ...
      meanCorrRight_cond1 <- get_meanCorrPartial_value(
      ...
      meanCorrRight_cond2 <- get_meanCorrPartial_value(
    ...
    if(tad_data$nGenes_left > 0) {
      ...
      meanCorrLeft_all <- get_meanCorrPartial_value(
      ...
      meanCorrLeft_cond1 <- get_meanCorrPartial_value(
      ...
      meanCorrLeft_cond2 <- get_meanCorrPartial_value(
