...
#####>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<>>>>>>>><<< 19.08.20: add purity filter !!! # 27.08.20 for random
...
stopifnot(setequal(names(emp_pval_logFC), basename(ds_TADs_to_keep) ))
stopifnot( basename(ds_TADs_to_keep) %in% names(emp_pval_intraCorr))
emp_pval_intraCorr <- emp_pval_intraCorr[names(emp_pval_intraCorr) %in% basename(ds_TADs_to_keep)]
stopifnot(setequal(names(emp_pval_logFC), names(emp_pval_intraCorr) ))
#####>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<>>>>>>>><<<
